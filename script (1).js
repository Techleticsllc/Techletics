(function () {
  ...
  function drawWordmark(ctx, size) {
    const fontReady =
      document.fonts && document.fonts.check && document.fonts.check('700 40px "Space Grotesk"');
    const displayFont = fontReady ? "'Space Grotesk', sans-serif" : "'Arial Black', Arial, sans-serif";
    const bodyFont = fontReady ? "'Inter', sans-serif" : "Arial, sans-serif";

    ctx.save();
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";

    // Faint printed backing plate so the type reads clearly over the seam lines
    ctx.fillStyle = "rgba(0,0,0,0.22)";
    ctx.fillRect(size * 0.16, size * 0.415, size * 0.68, size * 0.135);

    // TECHLETICS — bold orange print
    const grad = ctx.createLinearGradient(size * 0.18, 0, size * 0.82, 0);
    grad.addColorStop(0, "#FF8A5C");
    grad.addColorStop(1, "#FF5B2E");
    ctx.fillStyle = grad;
    ctx.font = `700 ${Math.round(size * 0.086)}px ${displayFont}`;
    try { ctx.letterSpacing = `${Math.round(size * 0.009)}px`; } catch (e) {}
    ctx.fillText("TECHLETICS", size * 0.5, size * 0.462);

    // Tagline
    ctx.font = `500 ${Math.round(size * 0.03)}px ${bodyFont}`;
    ctx.fillStyle = "rgba(255,148,104,0.95)";
    try { ctx.letterSpacing = `${Math.round(size * 0.003)}px`; } catch (e) {}
    ctx.fillText("Your Game, Upgraded", size * 0.5, size * 0.512);
  ...
  function initBallScene(canvas, opts) {
    ...
    if (document.fonts && document.fonts.ready) {
      document.fonts.ready.then(() => {
        const fresh = createBallTextures();
        mat.map.dispose();
        mat.emissiveMap.dispose();
        mat.map = fresh.baseTex;
        mat.emissiveMap = fresh.emissiveTex;
